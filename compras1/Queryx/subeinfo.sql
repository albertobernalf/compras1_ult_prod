...
create table aborrar
as
SELECT translate(btrim(num_identificacion::text),'óÓáÁéÉíÍúÚñÑ'::text,'oOaAeEiIuUnN'::text) num_identificacion   , translate(btrim(nom_usuario::text),'óÓáÁéÉíÍúÚñÑ'::text,'oOaAeEiIuUnN'::text) nom_usuario ,
	translate(btrim(clave_usuario::text),'óÓáÁéÉíÍúÚñÑ'::text,'oOaAeEiIuUnN'::text) clave_usuario ,translate(btrim(carg_usuario::text),'óÓáÁéÉíÍúÚñÑ'::text,'oOaAeEiIuUnN'::text) carg_usuario , 'A' estadoReg, 1 sede ,'S' perfil
...
select * from aborrar order by num_identificacion; -- 68
select num_identificacion, clave_usuario, count(*) from aborrar group by  num_identificacion, clave_usuario having count(*) = 1  order by num_identificacion;

-- Seria esta la CONDENSACION PARA QUE NIO QUEDEN REPETIDOS LOS USUAIOS Y CON UNA SOLA CLAVE

insert into solicitud_usuarios (num_identificacion, nom_usuario, clave_usuario,carg_usuario, estadoReg,sede_id, perfil)
select a.num_identificacion,a.nom_usuario, a.clave_usuario, a.carg_usuario, a.estadoReg, a.sede, a.perfil  from aborrar a where a.clave_usuario in (select max(b.clave_usuario) from aborrar b where b.num_identificacion = a.num_identificacion) order by a.num_identificacion

-- OJO INGRESAR A JEISON MILINA PERFIL VALIDACION
-- OJO ACTUALIZAR PERFILES DE COMPRAS Y ALMACEN 
