select * from imhotep_usuarios where cod_usuario='ADM001';
B4s32023*
...
select * from solicitud_usuarios where nom_usuario like ('%CORE%');
...
select * from solicitud_ordenesCompra where id=16;
select * from solicitud_usuarios WHERE NOM_USUARIO LIKE ('%MOLINA%');
select * from solicitud_solicitudes;	
select * from solicitud_solicitudesdetalle where "ordenCompra_id" = 15;	
update solicitud_solicitudesdetalle set "ordenCompra_id"=null where solicitud_id=1;
...
update  solicitud_usuarios set perfil = 'C' , clave_usuario ='12345' where id = 6;
update  solicitud_usuarios set  clave_usuario ='12345' where num_identificacion  = '1016069653';

update  solicitud_usuarios set perfil = 'C'  where num_identificacion = '52973594';

update  solicitud_usuarios set perfil = 'A' where id = 11;
update  solicitud_usuarios set perfil = 'V' where id = 69;
select * from solicitud_usuarios order by id;
...
SELECT *
FROM public.solicitud_usuarios 
WHERE  num_identificacion = '1016069653'

delete from  public.solicitud_usuarios WHERE estadoReg = 'A' and num_identificacion = '1016069653' and perfil = 'S'
select * from solicitud_proveedores;

-- Query para raer los datos de una Orden de Compra
-- 1er query

select c.id, c."fechaElab" fechaElab, c."fechaRevi" fechaRevi, c."fechaApro" fechaApro, c.contacto contacto,c."entregarEn" entregarEn, c.telefono telefono,
c.opciones opciones_id,case when c.opciones = 'A' then 'Anticipo' when c.opciones = 'C' then 'Contra Entrega' when c.opciones = 'N' then 'Noventa dias'  end opciones , c."valorBruto" valorBruto, c.descuento descuento, c."valorParcial" valorParcial, c.iva iva, c."valorTotal" valortotal,
c.observaciones observaciones ,c.aprobo_id aprobo_id,usu1.nom_usuario nombreAprobo,c."aproboCompraStaff_id" aproboCompraStaff_id, c.area_id area_id,area.area area,
c.elaboro_id elaboro_id,usu2.nom_usuario nombreElaboro,c."entragaMercancia_id" entragaMercancia_id,usu3.nom_usuario nombreEntregaMercancia,
c.proveedor_id proveedor_id, prov.proveedor proveedor, c."recibeMercancia_id" recibeMercancia_id,usu4.nom_usuario nombrerecibeMercancia,  c."responsableCompra_id",
usu5.nom_usuario responsableCompra,
 c.revizo_id revizo_id, usu6.nom_usuario nombreRevizo
 from solicitud_ordenescompra c 
left join solicitud_usuarios usu1 on (usu1.id = c.aprobo_id)
left join solicitud_usuarios usu2 on (usu2.id = c.elaboro_id)
left join solicitud_usuarios usu3 on (usu3.id = c."entragaMercancia_id")
left join solicitud_usuarios usu4 on (usu4.id = c."recibeMercancia_id")
left join solicitud_usuarios usu5 on (usu5.id = c."responsableCompra_id")
left join solicitud_usuarios usu6 on (usu6.id = c.revizo_id)
inner join solicitud_areas area on (area.id = c.area_id)
inner join solicitud_proveedores prov on (prov.id = c.proveedor_id)
where c.id=17;


 id,fechaElab, fechaRevi, fechaApro,contacto, entregarEn,  telefono, opciones , valorBruto, descuento,valorParcial, iva, valortotal, observaciones ,aprobo_id, nombreAprobo,aproboCompraStaff_id, area_id,area, elaboro_id, nombreElaboro,entragaMercancia_id, nombreEntregaMercancia,proveedor_id, proveedor, recibeMercancia_id,nombrerecibeMercancia,  responsableCompra_id, responsableCompra,revizo_id,  nombreRevizo
select * from solicitud_ordenescompra;

-- 2do query

select sol0.item item, art.articulo articulo, des.nombre descripcion, pre.nombre presentacion,sol0.iva,sol0."solicitadoOrdenCantidad" solicitadoOrdenCantidad, sol0."recibidoOrdenCantidad" recibidoOrdenCantidad ,
sol0."valorUnitario" valorUnitario,
sol0."solicitadoOrdenValor" solicitadoOrdenValor,
 sol0."recibidoOrdenValor" recibidoOrdenValor
from solicitud_solicitudesdetalle sol0
inner join solicitud_descripcionCompra des on (des.id = sol0.descripcion_id)
inner join solicitud_presentacion pre on (pre.id = sol0.presentacion_id)
inner join solicitud_articulos art on (art."codregArticulo" = sol0.producto)
where sol0."ordenCompra_id" = 17
order by sol0.item;



id,  fechaElab,fechaRevi, fechaApro, contacto, entregarEn, telefono, opciones_id, opciones , valorBruto, descuento, valorParcial, iva, valortotal, 
 observaciones ,aprobo_id, nombreAprobo, aproboCompraStaff_id,  area_id, area, elaboro_id, nombreElaboro,entragaMercancia_id,nombreEntregaMercancia, proveedor_id,
 proveedor,  recibeMercancia_id, nombrerecibeMercancia,  responsableCompra_id, responsableCompra,  revizo_id, nombreRevizo from solicitud_ordenescompra c left join solicitud_usuarios usu1 on (usu1.id = c.aprobo_id) left join solicitud_usuarios usu2 on (usu2.id = c.elaboro_id) left join solicitud_usuarios usu3 on (usu3.id = c."entragaMercancia_id") left join solicitud_usuarios usu4 on (usu4.id = c."recibeMercancia_id") left join solicitud_usuarios usu5 on (usu5.id = c."responsableCompra_id") left join solicitud_usuarios usu6 on (usu6.id = c.revizo_id) inner join solicitud_areas area on (area.id = c.area_id) inner join solicitud_proveedores prov on (prov.id = c.proveedor_id) where c.id=17


item, articulo, descripcion, presentacion,iva, solicitadoOrdenCantidad, recibidoOrdenCantidad ,valorUnitario, solicitadoOrdenValor, recibidoOrdenValor